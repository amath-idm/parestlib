...
def asd(function, x, args=None, stepsize=0.1, sinc=2, sdec=2, pinc=2, pdec=2,
    ...
    xorig = sc.dcp(x) # Keep the original x, just in case
    ...
    while True:
        ...
        xnew = sc.dcp(x) # Initialize the new parameter set
        ...
        if verbose >= 2: print(offset + label + ' step %i (%0.1f s) %s (orig: %s | best:%s | new:%s | diff:%s)' % ((count, time() - start, flag) + sc.sigfig([fvalorig, fvalold, fvalnew, fvalnew - fvalold])))
        ...
        if (time() - start) > maxtime:
            exitreason = 'Time limit reached (%s > %s)' % sc.sigfig([(time()-start), maxtime])
            break
        if (count > stalliters) and (abs(np.mean(abserrorhistory)) < abstol): # Stop if improvement is too small
            exitreason = 'Absolute improvement too small (%s < %s)' % sc.sigfig([np.mean(abserrorhistory), abstol])
            break
        if (count > stalliters) and (sum(relerrorhistory) < reltol): # Stop if improvement is too small
            exitreason = 'Relative improvement too small (%s < %s)' % sc.sigfig([np.mean(relerrorhistory), reltol])
            break
    ...
    # Return
    if verbose >= 2: print('=== %s %s (%i steps, orig: %s | best: %s | ratio: %s) ===' % ((label, exitreason, count) + sc.sigfig([fvals[0], fvals[-1], fvals[-1] / fvals[0]])))
    output = sc.objdict()
...
def bsd(function, x, verbose=True):
    '''
